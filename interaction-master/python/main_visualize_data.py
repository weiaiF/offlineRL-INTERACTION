...
def update_plot_with_param(timestamp,timestamp_min,timestamp_max, other_patches_dict, other_vehicle_polygon, ego_patches_dict,text_dict, axes,fig, title_text, others_track_dictionary, ego_state_dict,ego_shape_dict, pedestrian_dictionary):
    # update text and tracks based on current timestamp
    assert(timestamp <= timestamp_max), "timestamp=%i" % timestamp
    assert(timestamp >= timestamp_min), "timestamp=%i" % timestamp
    assert(timestamp % dataset_types.DELTA_TIMESTAMP_MS == 0), "timestamp=%i" % timestamp
    ...
    # plot ego vehicles
    tracks_vis.update_objects_plot_ego(ego_patches_dict, text_dict, axes,ego_shape_dict=ego_shape_dict,
                                   ego_motionstate_dict=ego_state_dict)
    fig.canvas.draw()

# a version needed param
def update_plot_with_param_new(timestamp,timestamp_min,timestamp_max, other_patches_dict, other_vehicle_polygon, ego_patches_dict,text_dict, axes,fig, title_text, others_track_dictionary, ego_state_dict,ego_shape_dict, pedestrian_dictionary):
    # update text and tracks based on current timestamp
    assert(timestamp <= timestamp_max), "timestamp=%i" % timestamp
    assert(timestamp >= timestamp_min), "timestamp=%i" % timestamp
    assert(timestamp % dataset_types.DELTA_TIMESTAMP_MS == 0), "timestamp=%i" % timestamp
...
class FrameControlButton(object):
    ...
    def on_click(self, event):
        global timestamp, timestamp_min, timestamp_max, playback_stopped, patches_dict, text_dict
...
if __name__ == "__main__":
    ...
    button_pause = FrameControlButton([0.82, 0.05, 0.1, 0.05], 'reset')
