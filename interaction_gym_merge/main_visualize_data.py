...
import sys
sys.path.append('/home/developer/workspace/interaction-dataset-master/python/')
...
def update_plot_with_param(timestamp,timestamp_min,timestamp_max, patches_dict, text_dict, axes,fig, title_text, others_track_dictionary, ego_state_dict,ego_shape_dict, pedestrian_dictionary):
    # update text and tracks based on current timestamp
    assert(timestamp <= timestamp_max), "timestamp=%i" % timestamp
    assert(timestamp >= timestamp_min), "timestamp=%i" % timestamp
    assert(timestamp % dataset_types.DELTA_TIMESTAMP_MS == 0), "timestamp=%i" % timestamp
    ...
    title_text.set_text("\nts = {} / {} ({:.2f}%)".format(timestamp, timestamp_max, percentage))
    # plot others
    tracks_vis.update_objects_plot_without_ego(timestamp, patches_dict, text_dict, axes,
                                   other_track_dict=others_track_dictionary, pedest_dict=pedestrian_dictionary)
    # plot ego vehicles
    tracks_vis.update_objects_plot_ego(patches_dict, text_dict, axes,ego_shape_dict=ego_shape_dict,
