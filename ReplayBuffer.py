...
class ReplayBuffer(object):
	...
	def sample_part_state(self, batch_size):
		
		return (torch.FloatTensor(self.state[0:batch_size]).to(self.device))

	def sample_part_state_new(self, initial, batch_size):
		
		return (torch.FloatTensor(self.state[initial:initial + batch_size]).to(self.device))

	def sample_part_action(self, batch_size):
		
		return (torch.FloatTensor(self.action[0:batch_size]).to(self.device))

	def sample_part_action_new(self, initial, batch_size):
		
		return (torch.FloatTensor(self.action[initial:initial + batch_size]).to(self.device))
