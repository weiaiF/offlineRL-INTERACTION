...
class BCQ(object):
	...
	def select_action(self, state):		
		with torch.no_grad():
			state = torch.FloatTensor(state.reshape(1, -1)).repeat(1, 1).to(self.device)
	...
	def train(self, replay_buffer, iterations, batch_size=256):
		...
		for it in range(iterations):
			...
			loss_list[0].append(actor_loss)
			loss_list[1].append(critic_loss)
			loss_list[2].append(vae_loss)
			# print(f"actor_loss: {actor_loss}, critic_loss:{critic_loss}, vae_loss:{vae_loss}")
